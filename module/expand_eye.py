...
#     x_center = np.mean(points[:, 0]).astype(int)
#     y_center = np.mean(points[:, 1]).astype(int)
...
# # def expand_region(image,points,scale_factor):
    
# #     # Convex Hull 생성 (눈 형태를 따라감)
...
# #         (int(x_center + (p[0] - x_center) * scale_factor),
# #          int(y_center + (p[1] - y_center) * scale_factor)) for p in points
# #     ], np.int32)
